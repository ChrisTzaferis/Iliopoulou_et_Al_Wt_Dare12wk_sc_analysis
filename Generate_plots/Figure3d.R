...
total_up_down_bar$cluster <- factor(total_up_down_bar$cluster, levels = total_degs$cluster[order(total_degs$TOTAL, decreasing = T)])
total_up_down_bar$state <- factor(total_up_down_bar$state, levels = c("up", "down"))

ggplot(total_up_down_bar, aes(fill=state, y=n, x=cluster)) + 
  geom_bar(position="stack", stat="identity") + 
  scale_fill_manual(values = c("red3", "royalblue"))+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), text = element_text(size = 29), plot.title = element_text(hjust = 0.5)) +
  labs(y="n of DEGs", x="", title = expression(italic(Tnf^{DARE}) ~ "vs" ~ italic(Tnf^{"+/+"})) )
ggsave("Fig3d.pdf", width = 18, height = 14, units = "in", dpi = 600)
