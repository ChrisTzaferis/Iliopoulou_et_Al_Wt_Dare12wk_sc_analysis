...
dare_table <- subsetCommunication(cellchat.LS, slot.name = "netP", sources.use = c("Pdgfra_low", "Trophocytes", "Telocytes"))
...
wt_table <- subsetCommunication(cellchat.NL, slot.name = "netP", sources.use = c("Pdgfra_low", "Trophocytes", "Telocytes"))
...
#dare dotplot
df_dare12w$prob[df_dare12w$prob > 0.14] <- 0.14
...
x_axis_colors <- ifelse(
  df_common$log2_prob_ratio < -0.1607403, "blue",
  ifelse(df_common$log2_prob_ratio <= 0.3235806, "grey", "red")
)
...
ggsave(plot = combined_plot, "Fig4d.pdf", width = 52, height = 15, units = "cm", dpi = 600)
